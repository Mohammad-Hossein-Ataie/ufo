...
function parseInput(productId: string, body: unknown): AdminProductInput {
  ...
  return {
    ...
    wholesalePriceRial: value.wholesalePriceRial === undefined ? undefined : Number(value.wholesalePriceRial),
    wholesaleEnabled: Boolean(value.wholesaleEnabled),
    cartonSize: value.cartonSize === undefined ? undefined : Number(value.cartonSize),
    minWholesaleCartonCount: value.minWholesaleCartonCount === undefined ? undefined : Number(value.minWholesaleCartonCount),
    ...
    restockThreshold: value.restockThreshold === undefined ? undefined : Number(value.restockThreshold),
...
export async function PATCH(req: Request, { params }: { params: Promise<{ productId: string }> }) {
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "به‌روزرسانی محصول ناموفق بود." }, { status: 400 });
  }
}
