...
export function AdminOrderChatClient({ orderId }: { orderId: string }) {
  ...
  async function loadMessages() {
    const response = await fetch(`/api/admin/chat?orderId=${encodeURIComponent(orderId)}`, { cache: "no-store" });
  ...
  return (
    <section className="rounded-md border border-[#D7DDE4] bg-white p-5">
      ...
      <div className="mt-4 grid max-h-96 gap-3 overflow-y-auto">
        {messages.length === 0 ? <p className="text-sm text-[#5F6C79]">هنوز پیامی ثبت نشده است.</p> : null}
      ...
      <form onSubmit={submit} className="mt-4 grid gap-3">
        <Textarea value={body} onChange={(event) => setBody(event.target.value)} placeholder="پاسخ ادمین" />
