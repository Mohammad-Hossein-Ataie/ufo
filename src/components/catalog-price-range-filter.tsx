...
export function CatalogPriceRangeFilter({
  ...
  const [from, setFrom] = useState(() => clampValue(parseValue(defaultMin, min), min, max));
  const [to, setTo] = useState(() => clampValue(parseValue(defaultMax, max), min, max));
  const formatter = useMemo(() => new Intl.NumberFormat("fa-IR"), []);
  const clampedFrom = clampValue(from, min, max);
  const clampedTo = clampValue(to, min, max);
  const lower = Math.min(clampedFrom, clampedTo);
  const upper = Math.max(clampedFrom, clampedTo);
  const range = Math.max(1, max - min);
  const lowerPercent = clampValue(((lower - min) / range) * 100, 0, 100);
  const upperPercent = clampValue(((upper - min) / range) * 100, 0, 100);
  ...
  const thumbClass = `${thumbColor} pointer-events-none absolute inset-x-0 top-1/2 z-20 h-2 w-full min-w-0 max-w-full -translate-y-1/2 appearance-none bg-transparent focus:outline-none focus-visible:[&::-moz-range-thumb]:ring-4 focus-visible:[&::-moz-range-thumb]:ring-cyan-300/30 focus-visible:[&::-webkit-slider-thumb]:ring-4 focus-visible:[&::-webkit-slider-thumb]:ring-cyan-300/30 [&::-moz-range-thumb]:pointer-events-auto [&::-moz-range-thumb]:h-5 [&::-moz-range-thumb]:w-5 [&::-moz-range-thumb]:cursor-grab [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border-2 [&::-moz-range-thumb]:border-white [&::-moz-range-thumb]:bg-[var(--range-thumb)] [&::-moz-range-thumb]:shadow-lg [&::-moz-range-track]:bg-transparent [&::-webkit-slider-runnable-track]:h-2 [&::-webkit-slider-runnable-track]:bg-transparent [&::-webkit-slider-thumb]:pointer-events-auto [&::-webkit-slider-thumb]:mt-[-6px] [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:cursor-grab [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-white [&::-webkit-slider-thumb]:bg-[var(--range-thumb)] [&::-webkit-slider-thumb]:shadow-lg`;
  ...
  return (
    <fieldset className="grid w-full min-w-0 max-w-full gap-4 rounded-md border border-current/15 p-3">
      ...
      <div className="grid min-w-0 gap-3 pt-1">
        <div className="relative h-10 w-full min-w-0 max-w-full px-1">
          <div
